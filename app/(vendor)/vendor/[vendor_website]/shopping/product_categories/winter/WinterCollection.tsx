...
const VendorWinterCollection: React.FC = () => {
  ...
  if (!collections?.winter || collections.winter.length === 0) {
    return (
      <div className="text-center py-8">
        <h2 className="text-2xl font-bold text-foreground">
          No products found in your winter collection.
        </h2>
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(collections.winter.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = collections.winter.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(startIndex + ITEMS_PER_PAGE, collections.winter.length)} of{" "}
        {collections.winter.length} products
      </div>
    </div>
  );
};
