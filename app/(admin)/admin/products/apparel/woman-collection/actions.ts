...
export async function fetchWomenCollections(
  type?: string,
  searchQuery?: string
  ...
  try {
    ...
    if (user.role !== "ADMIN") {
      throw new Error("Only admins can fetch women collections.");
    }
    ...
    const baseWhereCondition: Prisma.ProductWhereInput = {
      OR: [
        {
          categories: { contains: "Apparel Collection > Women" },
        },
        { categories: { contains: "Women" } },
      ],
    };
    ...
    revalidatePath("/admin/products/apparel/woman-collection");

    return { success: true, data: womenProducts };
  } catch (error) {
