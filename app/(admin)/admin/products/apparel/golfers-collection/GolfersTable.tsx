...
import { fetchGolfersCollections } from "./actions";

const GolfersTable = () => {
  const [golfersProducts, setGolfersProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    const loadGolfersProducts = async () => {
      setIsLoading(true);
      const result = await fetchGolfersCollections(undefined, searchQuery);
      if (result.success) {
        setGolfersProducts(result.data);
        setError(null);
      } else {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="m-6 flex space-x-9 items-center">
        <h1 className="text-2xl font-extrabold">GOLFERS COLLECTION</h1>
        <div className="w-52">
