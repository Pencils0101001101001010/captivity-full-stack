  ...
  try {
    ...
    if (heroProduct && heroProduct.imageUrl) {
      ...
      const heroImage = images.find(
        url =>
          url.includes("model") ||
          url.includes("Multifunctional Headwear") ||
          url.includes("Multifunctional Headwear")
      );
...
export async function fetchMultifunctionalHeadwear(
  type?: string,
  searchQuery?: string,
  page: number = 1,
  pageSize: number = 9
  ...
  try {
    ...
    const [multifunctionalHeadwear, totalCount] = await Promise.all([
    ...
    return { success: true, data: multifunctionalHeadwear, totalCount };
  } catch (error) {
