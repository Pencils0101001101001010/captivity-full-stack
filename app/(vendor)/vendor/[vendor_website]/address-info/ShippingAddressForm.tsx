...
import { updateShippingAddress, getUserDetails } from "./actions";
    ...
    const loadUserData = async () => {
      try {
        ...
        if (result.success && result.data) {
          // Use shipping address if it exists, otherwise use main user data
          const shippingData = result.data.shippingAddress;
          form.reset({
            firstName: shippingData?.firstName || result.data.firstName || "",
            lastName: shippingData?.lastName || result.data.lastName || "",
            email: shippingData?.email || result.data.email || "",
            phone: shippingData?.phoneNumber || result.data.phoneNumber || "",
            companyName:
              shippingData?.companyName || result.data.companyName || "",
            countryRegion: shippingData?.country || result.data.country || "",
            streetAddress:
              shippingData?.streetAddress || result.data.streetAddress || "",
            apartmentSuite:
              shippingData?.addressLine2 || result.data.addressLine2 || "",
            townCity: shippingData?.townCity || result.data.townCity || "",
            province: shippingData?.suburb || result.data.suburb || "",
            postcode: shippingData?.postcode || result.data.postcode || "",
          });
        }
      } catch (error) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Error",
          description: result.error || "Failed to update shipping address.",
          variant: "destructive",
          duration: 5000,
        });
      }
