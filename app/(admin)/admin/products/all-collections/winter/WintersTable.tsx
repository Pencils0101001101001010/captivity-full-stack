...
const WinterTable = () => {
  const [winterProducts, setWinterProducts] = useState<Product[]>([]);
  ...
  useEffect(() => {
    const loadWinterProducts = async () => {
      setIsLoading(true);
      const result = await fetchWinterCollections(undefined, searchQuery);
      if (result.success) {
        setWinterProducts(result.data);
        setError(null);
      } else {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="m-6 flex space-x-9 items-center">
        <h1 className="text-2xl font-extrabold">WINTER COLLECTION</h1>
        <div className="w-52">
