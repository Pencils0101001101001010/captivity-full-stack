...
const VendorLeisureCollection: React.FC = () => {
  ...
  if (!collections?.leisure || collections.leisure.length === 0) {
    return (
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(collections.leisure.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = collections.leisure.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(startIndex + ITEMS_PER_PAGE, collections.leisure.length)} of{" "}
        {collections.leisure.length} products
      </div>
    </div>
  );
};
