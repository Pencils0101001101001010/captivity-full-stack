...
import useJackets from "./useJackets";
...
const HoodiesProductList: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const { products, loading, error } = useJackets();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[200px]">
        ...
        <p className="text-muted-foreground">Loading Jackets collection...</p>
      </div>
    );
  }
  ...
  return (
    <section className="container mx-auto my-8">
      <HeroSection featuredImage={featuredImage} categoryName="Jackets" />
      ...
      <div className="flex flex-col md:flex-row gap-6 relative">
        ...
        <h1 className="text-gray-500 text-xl mb-6">Jackets</h1>

          {products.length === 0 ? (
            <div className="text-center py-8">
              <h2 className="text-2xl font-bold text-foreground">
                No Jackets available in the collection.
              </h2>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                {paginatedProducts.map((product: ProductWithFeaturedImage) => (
                  <Link
                    href={`/products/jackets/${product.id}`}
                    key={product.id}
                  >
...
export default HoodiesProductList;