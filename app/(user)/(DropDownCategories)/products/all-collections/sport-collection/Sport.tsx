...
const SportProductList: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const { products, loading, error } = useSport();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[200px]">
        ...
        <p className="text-muted-foreground">Loading sport collection...</p>
      </div>
    );
  };
  ...
  return (
    <section className="container mx-auto my-8">
      <HeroSection featuredImage={featuredImage} categoryName="SPORT-COLLECTION" />
      ...
      <div className="flex flex-col md:flex-row gap-6 relative">
        ...
        <h1 className="text-gray-500 text-xl mb-6">Sport Collection</h1>
          {products.length === 0 ? (
            <div className="text-center py-8">
              <h2 className="text-2xl font-bold text-foreground">
                No sport products available in the collection.
              </h2>
            </div>
          ) : (
            <>
...
export default SportProductList;