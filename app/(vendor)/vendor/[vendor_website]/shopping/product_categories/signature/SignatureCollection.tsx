...
const VendorSignatureCollection: React.FC = () => {
  ...
  if (!collections?.signature || collections.signature.length === 0) {
    return (
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(collections.signature.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = collections.signature.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(startIndex + ITEMS_PER_PAGE, collections.signature.length)} of{" "}
        {collections.signature.length} products
      </div>
    </div>
  );
};
