...
import { fetchIndustrialCollections } from "./actions";
import SearchField from "@/app/(admin)/_components/SearchField";
import Link from "next/link";
import { Eye, Pencil, Trash } from "lucide-react";

const IndustrialTable = () => {
  const [industrialProducts, setIndustrialProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  const loadIndustrialProducts = async (query: string = "") => {
    setIsLoading(true);
    const result = await fetchIndustrialCollections(undefined, query);
    if (result.success) {
      setIndustrialProducts(result.data);
      setError(null);
    } else {
    ...
    <div>
      <div className="overflow-x-auto mt-4">
        <div className="m-6 flex space-x-9 items-center">
          <h1 className="text-2xl font-extrabold">INDUSTRIAL COLLECTION</h1>
          <div className="w-52">
