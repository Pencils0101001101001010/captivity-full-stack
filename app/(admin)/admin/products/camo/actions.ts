...
// Fetch camo collection products for table
export async function fetchCamoCollectionTable(): Promise<FetchCamoCollectionResult> {
  try {
    ...
    console.error("Error fetching camo collection for table:", error);
    return {
      success: false,
      error:
...
export async function toggleProductPublish(
  ...
  try {
    ...
    revalidatePath("/admin/products/camo");
    revalidatePath("/customer/shopping/product_categories/camo");
