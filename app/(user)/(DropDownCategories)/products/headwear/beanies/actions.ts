...
type HeroImageResult =
  ...
  | { success: false; error: string };
//! kan meer imageUrl opsies hier maak met om vir nog fotos te soek "const multiImageResults = [{ ... }]
...
export async function fetchBeanies(
  type?: string,
  searchQuery?: string,
  page: number = 1,
  pageSize: number = 9
  ...
  try {
    const baseWhereCondition: Prisma.ProductWhereInput = {
      ...
      OR: [
        {
          categories: {
            contains: "Headwear Collection > Beanies",
          },
        },
        { categories: { contains: "Beanies" } },
      ],
    };
    ...
    const [beanies, totalCount] = await Promise.all([
    ...
    return { success: true, data: beanies, totalCount };
  } catch (error) {
