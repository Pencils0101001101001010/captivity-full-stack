...
import { fetchCamoCollections } from "./actions";
import { Eye, Pencil, Trash } from "lucide-react";
import Link from "next/link";
import SearchField from "@/app/(admin)/_components/SearchField";

const CamoTable = () => {
  const [camoProducts, setCamoProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    const loadCamoProducts = async () => {
      setIsLoading(true);
      const result = await fetchCamoCollections(undefined, searchQuery);
      if (result.success) {
        setCamoProducts(result.data);
        setError(null);
      } else {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="m-6 flex space-x-9 items-center">
        <h1 className="text-2xl font-extrabold">CAMO COLLECTION</h1>
        <div className="w-52">
