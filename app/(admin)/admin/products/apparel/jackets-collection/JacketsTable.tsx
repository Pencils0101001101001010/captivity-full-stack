...
import { fetchJacketCollections } from "./actions";

const JacketTable = () => {
  const [jacketProducts, setJacketProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  useEffect(() => {
    const loadJacketProducts = async () => {
      setIsLoading(true);
      const result = await fetchJacketCollections(undefined, searchQuery);
      if (result.success) {
        setJacketProducts(result.data);
        setError(null);
      } else {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="m-6 flex space-x-9 items-center">
        <h1 className="text-2xl font-extrabold">JACKET COLLECTION</h1>
        <div className="w-52">
