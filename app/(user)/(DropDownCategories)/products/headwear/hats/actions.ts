  ...
  try {
    ...
    if (heroProduct && heroProduct.imageUrl) {
      ...
      const heroImage = images.find(
        url =>
          url.includes("model") || url.includes("Hats") || url.includes("hats")
      );
...
const EXCLUDED_TERMS = ["bucket", "bush"];
export async function fetchHats(
  type?: string,
  searchQuery?: string,
  page: number = 1,
  pageSize: number = 9
  ...
  try {
    const baseWhereCondition: Prisma.ProductWhereInput = {
      published: true,
      OR: [
        { categories: { contains: "Headwear Collection > Hats" } },
        { categories: { contains: "Hats" } },
      ],
      NOT: EXCLUDED_TERMS.map(term => ({
    ...
    const [Hats, totalCount] = await Promise.all([
    ...
    return { success: true, data: Hats, totalCount };
  } catch (error) {
