...
export async function fetchPreCurvedPeaks(
  ...
  try {
    const baseWhereCondition: Prisma.ProductWhereInput = {
      published: true,
      OR: [
        { categories: { contains: "Headwear Collection > Pre-Curved Peaks" } },
    ...
    const [PreCurvedPeaksProducts, totalCount] = await Promise.all([
    ...
    return { success: true, data: PreCurvedPeaksProducts, totalCount };
  } catch (error) {
    console.error("Error fetching Pre Curved Peaks:", error);
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "An unexpected error occurred",
