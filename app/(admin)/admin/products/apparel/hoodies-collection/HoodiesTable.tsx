...
import { fetchHoodieCollections } from "./actions";
...
const HoodiesTable = () => {
  ...
  useEffect(() => {
    const loadHoodieProducts = async () => {
      setIsLoading(true);
      const result = await fetchHoodieCollections(undefined, searchQuery);
      if (result.success) {
        setHoodieProducts(result.data);
        setError(null);
      } else {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="m-6 flex space-x-9 items-center">
        <h1 className="text-2xl font-extrabold">HOODIE COLLECTION</h1>
        <div className="w-52">
