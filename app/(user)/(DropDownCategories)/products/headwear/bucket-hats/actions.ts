...
export async function fetchBucketCaps(
  type?: string,
  searchQuery?: string,
  page: number = 1,
  pageSize: number = 9
): Promise<FetchBucketHatsResult> {
  try {
    const baseWhereCondition: Prisma.ProductWhereInput = {
      ...
      OR: [
        {
          categories: {
            contains: "Headwear Collection > Bucket Hats",
          },
        },
        { categories: { contains: "Bucket Hats" } },
      ],
    };
    ...
    const [bucketCaps, totalCount] = await Promise.all([
    ...
    return { success: true, data: bucketCaps, totalCount };
  } catch (error) {
    console.error("Error fetching multifunctionalHeadwear:", error);
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "An unexpected error occurred",
