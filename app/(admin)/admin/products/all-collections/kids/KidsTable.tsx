...
import { fetchKidCollections } from "./actions";
...
const KidsTable = () => {
  ...
  useEffect(() => {
    const loadKidProducts = async () => {
      setIsLoading(true);
      const result = await fetchKidCollections(undefined, searchQuery);
      if (result.success) {
        setKidProducts(result.data);
        setError(null);
      } else {
  ...
  return (
    <div className="overflow-x-auto">
      <div className="m-6 flex space-x-9 items-center">
        <h1 className="text-2xl font-extrabold">KIDS COLLECTION</h1>
        <div className="w-52">
