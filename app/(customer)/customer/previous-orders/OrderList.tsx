"use client";

import { Order } from "./types";
import { formatDate, getStatusColor } from "@/lib/utils";
import Link from "next/link";
import { OrderStatus } from "@prisma/client";
import Image from "next/image";

interface OrderHistoryProps {
  orders?: Order[];
  error?: string;
}

export function OrderHistory({ orders, error }: OrderHistoryProps) {
  if (error) {
    return <div className="text-red-500 text-center py-8">{error}</div>;
  }

  if (!orders?.length) {
    return (
      <div className="text-center py-8">
        <p className="mb-4">No order has been made yet.</p>
        <Link
          href="/products"
          className="inline-block bg-gray-700 hover:bg-gray-600 text-white px-6 py-2 rounded-md"
        >
          Browse products
        </Link>
      </div>
    );
  }

  return (
    <div className="space-y-6">
      {orders.map(order => (
        <div
          key={order.id}
          className="bg-gray-200 text-gray-700 rounded-lg p-6 shadow-sm"
        >
          {/* Order Header */}
          <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
            <div>
              <h3 className="text-lg font-medium">
                Order #{order.referenceNumber || order.id.slice(-8)}
              </h3>
              <p className="text-sm">Placed on {formatDate(order.createdAt)}</p>
              <p className="text-sm">
                Collection Method: {order.methodOfCollection}
              </p>
              <p className="text-sm">Branch: {order.captivityBranch}</p>
              {order.salesRep && (
                <p className="text-sm">Sales Rep: {order.salesRep}</p>
              )}
            </div>
            <div className="flex flex-col items-end">
              <span className="text-lg font-medium">
                R{order.totalAmount.toFixed(2)}
              </span>
              {/* Changed from p to div to avoid nesting violation */}
              <div className="flex items-center">
                <span
                  className={`px-3 py-1 rounded-full text-sm ${getStatusColor(
                    order.status
                  )}`}
                >
                  {order.status}
                </span>
              </div>
            </div>
          </div>

          {/* Delivery Information */}
          <div className="mb-4 text-sm">
            <h4 className="font-medium mb-2">Delivery Address:</h4>
            <div className="space-y-1">
              <p>
                {order.firstName} {order.lastName}
              </p>
              <p>{order.companyName}</p>
              <p>{order.streetAddress}</p>
              {order.apartmentSuite && <p>{order.apartmentSuite}</p>}
              <p>
                {order.townCity} {order.province}
              </p>
              <p>{order.postcode}</p>
              <p>{order.countryRegion}</p>
              <p>Phone: {order.phone}</p>
              <p>Email: {order.email}</p>
            </div>
          </div>

          {/* Order Items */}
          <div className="divide-y divide-gray-700">
            {order.orderItems.map(item => (
              <div key={item.id} className="py-4 flex items-center gap-4">
                <div className="flex-shrink-0 w-20 h-20">
                  <Image
                    src={item.variation.variationImageURL || "/placeholder.png"}
                    alt={item.variation.name}
                    className="w-full h-full object-cover rounded"
                    width={60}
                    height={60}
                  />
                </div>
                <div className="flex-grow">
                  <h4 className="">{item.variation.name}</h4>
                  <p className="text-sm">
                    {item.variation.color} / {item.variation.size}
                  </p>
                  <p className="text-sm">
                    SKU: {item.variation.sku}
                    {item.variation.sku2 && ` / ${item.variation.sku2}`}
                  </p>
                </div>
                <div className="text-right">
                  <p className="">R{item.price.toFixed(2)}</p>
                  <p className="text-sm">Qty: {item.quantity}</p>
                </div>
              </div>
            ))}
          </div>

          {/* Order Notes */}
          {order.orderNotes && (
            <div className="mt-4 p-4 bg-gray-700/20 rounded-md">
              <h4 className="text-sm font-medium mb-1">Order Notes:</h4>
              <p className="text-sm">{order.orderNotes}</p>
            </div>
          )}
        </div>
      ))}
    </div>
  );
}
