...
type FetchIndustrial =
  | { success: true; data: ProductWithFeaturedImage[] }
  | { success: false; error: string };

export async function fetchIndustrial(): Promise<FetchIndustrial> {
  try {
    const industrial = await prisma.product.findMany({
      where: {
        category: {
