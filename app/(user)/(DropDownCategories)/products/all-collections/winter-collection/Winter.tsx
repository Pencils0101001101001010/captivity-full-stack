...
const WinterProductList: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const { products, loading, error } = useWinter();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[200px]">
        ...
        <p className="text-muted-foreground">Loading winter collection...</p>
      </div>
    );
  };
  ...
  return (
    <section className="container mx-auto my-8">
      <HeroSection featuredImage={featuredImage} categoryName="WINTER-COLLECTION" />
      ...
      <div className="flex flex-col md:flex-row gap-6 relative">
        ...
        <h1 className="text-gray-500 text-xl mb-6">Winter Collection</h1>
          {products.length === 0 ? (
            <div className="text-center py-8">
              <h2 className="text-2xl font-bold text-foreground">
                No winter products available in the collection.
              </h2>
            </div>
          ) : (
            <>
...
export default WinterProductList;