...
export async function fetchTruckerCaps(
  type?: string,
  searchQuery?: string,
  page: number = 1,
  pageSize: number = 9
  ...
  try {
    const baseWhereCondition: Prisma.ProductWhereInput = {
      ...
      OR: [
        {
          categories: {
            contains: "Headwear Collection > Trucker Caps",
          },
        },
        { categories: { contains: "Trucker Caps" } },
      ],
    };
    ...
    const [truckerCaps, totalCount] = await Promise.all([
    ...
    return { success: true, data: truckerCaps, totalCount };
  } catch (error) {
